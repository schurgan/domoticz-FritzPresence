...
| Version | Note                                                                                     |
| ------- | ---------------------------------------------------------------------------------------- |
| 1.0.0   | Stability & Reliability Update, Improved overall stability and robustness, Fixed multiple edge cases in device creation and unit handling, Devices are now uniquely handled by MAC address (DeviceID) to prevent duplicates, Safer image handling (no error spam if icons or units are missing), Protected device renaming via Domoticz JSON API, Automatically disables renaming after HTTP 401 Unauthorized, Added error backoff mechanism to prevent log flooding and high load, Improved polling logic and recovery after Fritz!Box errors, Devices are updated only when values actually change, Safer “remove all devices” handling (MAC-based devices only), This release focuses on long-term stable operation without changing existing functionality|
